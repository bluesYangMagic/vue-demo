        ...
        <ul>
          <li v-for="(item,index) in goods" class="menu-item" :class="{'current': currentIndex === index}" @click="selectMenu(index)">
            <span class="text border-1px">
              <span class="icon" v-show="item.type > 0" :class="classArray[item.type]">
              </span>
              {{item.name}}
            </span>
          </li>
      ...
      <div class="foods-wrapper" ref="foodsWrapper">
        <ul>
          <li v-for="(item,index) in goods" class="foods-list foods-list-hook">
            ...
            <ul>
              <li v-for="(item,index) in item.foods" class="food-item border-1px">
                <div class="icon">
                  <img :src="item.icon" alt="食物" width="57" height="57">
                </div>
                <div class="content">
...
<style lang="stylus">
  ...
  .goods
    ...
    overflow: hidden
    .menu-wrapper
      flex: 0 0 80px // 等分 缩放 固定宽度
      width: 80px // 兼容问题 android
      background: #f3f5f7;
      .menu-item
        // 垂直居中 table 最好的布局
        display: table
